...
- Keep home row columns aligned with a shared template, reserve stable scrollbar gutters, and give closed native fields explicit hover/focus/disabled styles. The language picker shows the resolved language name; automatic detection remains a distinct option.
- Use native selects and dialogs. Number fields accept temporary drafts, apply on blur/Enter and cancel with Escape, retaining focus on Enter/Escape. Preview controls stay outside the image and remain available in fullscreen. PNG clearly identifies its lossless behavior.
