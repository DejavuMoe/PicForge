...
- Keep home row columns aligned with a shared template, reserve stable scrollbar gutters, and give closed native fields explicit hover/focus/disabled styles. The language picker contains only the five language names. Browser detection is the initial default, never a displayed option or policy label.
