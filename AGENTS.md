...
- The accepted image workbench is implemented. Preserve [the visual direction](docs/design/visual-direction.md), [tokens](docs/design/design-tokens.json) and [implementation/QA baseline](docs/UI_DESIGN.md). Keep visual changes separate from engine work; the entry is now a compact Landing Page with three tool links (latest user decision), followed by the existing workbench.
- Use one image-first workbench language across compression and Motion/Live Photo tools: predictable navigation/actions, neutral image-viewing surfaces, flat file rows and shared control families. Preserve per-tool queues, global/per-image setting provenance, compare/zoom, cancellation and downloads.
- The latest user review replaces the blur-only, oversized-whitespace design: use [the optical interaction system](docs/design/interaction-redesign.md). Landing has one bounded interactive particle/photographic scene, three clear tool entries and an expanded centered footer. Use real Hyalite edge refraction for selected navigation, the preview dock and appropriate overlay surfaces; never filter exported media or inspector text. Remove duplicate underline/border indicators. Keep motion purposeful, pointer targets stable, and retain reduced-motion/transparency fallbacks.
- Define semantic light/dark tokens, a small type/spacing/radius scale, one icon family, visible focus and adequately sized touch targets. Cover mobile reflow and all five locales; do not simply shrink desktop columns. Keep assets/fonts local or use system fonts; use the pinned local optical adapter and bounded home-only particle renderer rather than broad animation dependencies. Pause effects offscreen/hidden and detach them when a tool becomes inactive.
- Detect the browser language by default with English fallback. Persist only explicit language choices under `picforge.language`; URL previews and the legacy detector cache must not override future automatic detection. The latest footer annotation removes About PicForge: use copyright and GitHub, with sponsorship on a separate Landing footer line. Preserve all license files; do not add a raw license link or replacement About entry to the primary footer.
- Show generated concepts as proposals with illustrative data clearly distinguished from measured results. Do not use personal sample photos in external design/image-generation services. Keep private media and temporary screenshots outside commits; preserve reference attribution and distinguish visual concepts from implemented functionality.

- Prototype fidelity: the green P mark and platform icons, separate beveled tool panes, raised selection controls and horizontal dimension rows follow the supplied optical prototypes. Percentage sizing remains in Advanced settings. Both themes share component geometry; the Landing uses separate light/dark artwork assets with code-driven parallax.
