...
- Shared control regression subsets: `details,ranges,usability` cover row insets, controls and numeric settings. `player` uses temporary portrait/landscape JPEG/MP4 fixtures via `PICFORGE_PLAYER_FIXTURES` to check both media tools, sub-second progress, scrubbing and hidden cleanup; see the fixture contract in [UI validation](docs/UI_DESIGN.md).
...
- Branding uses the [folded P identity](docs/design/brand.md). The single SVG master is `packages/app/src/assets/logo.svg`; `pnpm assets:brand` regenerates icon and sharing-card exports. Preserve ordinary/maskable icon roles, favicon revision URLs, exact social-image dimensions and the service-worker shell entries when changing assets.
...
- Keep home row columns aligned with a shared template and symmetric insets (20 px desktop/tablet, 12 px mobile), reserve stable scrollbar gutters, and give shared controls explicit hover/focus/disabled styles. Range/number pairs share heights even when disabled. The language picker contains only the five language names. Browser detection is the initial default, never a displayed option or policy label.
- Use the shared themed combobox, including its popup, and styled dialogs. All web control states, hints and video playback controls use application styling; pointer clicks do not leave keyboard focus frames. Number fields accept temporary drafts, apply on blur/Enter and cancel with Escape, retaining focus on Enter/Escape. Preview controls stay outside the image and remain available in fullscreen. PNG clearly identifies its lossless behavior.
- In both media tools, the playback dock matches the visible video's left/right edges and directly follows its bottom edge. Never stretch it to the enclosing pane. Narrow players move the timer to a separate row; resizing and tool reactivation must recompute the fitted size.
- Photos have their own fitted strip with actual output pixel dimensions and fullscreen access. Match the paired dock height on desktop, keep download rows aligned, and retain a compact strip on mobile. Do not fill the photo side with decorative playback controls.
...
- GitHub is an icon link at the top right. Every normal page shares a footer: copyright left and Riven Cloud right on desktop; copyright above sponsorship, both centered, on mobile. Do not restore About or a raw license link to the primary footer. Preserve all license files and attribution, including removed historical components.
